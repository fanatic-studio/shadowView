...
- (void)layoutSubviews {
    ...
    if (_shadowRadius > 0) {
        if (_shadowSide & YCShadowSideTop) {
            frame.origin.y = _shadowRadius;
            frame.size.height-= _shadowRadius;
        }
        if (_shadowSide & YCShadowSideBottom) {
            frame.size.height-= _shadowRadius;
        }
        if (_shadowSide & YCShadowSideLeft) {
            frame.origin.x = _shadowRadius;
            frame.size.width-= _shadowRadius;
        }
        if (_shadowSide & YCShadowSideRight) {
            frame.size.width-= _shadowRadius;
        }
    }
    self.backContentView.frame = frame;
...
- (void) backToBack {
    [self sendSubviewToBack:_backContentView];
}
